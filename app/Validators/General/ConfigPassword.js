...
class Password {
  ...
  async authorize () {
    ...
    if (await Hash.verify(request.input('current_password'), auth.user.password)) {
      return true
    }

    session
      .flash({ danger: 'A senha atual para autorizar o procedimento está incorreta. Tente novamente.' })
