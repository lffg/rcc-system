...
class ValidateUser {
  ...
  static async validate (name = '', motto = false) {
    ...
    try {
      const { body: { motto: realMotto = '' } } = await got('https://www.habbo.com.br/api/public/users', {
        query: { name },
        cache: false,
        json: true
      })
      ...
      if (!!motto && realMotto !== motto) {
        result.error = 'Erro de autenticação: O código de confirmação não encontra-se na sua missão.'
      ...
      if (body.error === 'not-found') {
        ...
        result.error = 'Houve um erro ao tentar estabelecer conexão com a Habbo API. Tente novamente mais tarde.'
      }
    }
